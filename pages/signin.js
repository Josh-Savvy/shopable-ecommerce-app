  ...
  .box {
    ...
    .form {
      ...
      button {
        ...
        border-radius: 6px;
        background: #1e9700;
        background: -webkit-linear-gradient(to right, #1e9700, #1e9700);
        background: linear-gradient(to right, #1e9700, #1e9700);
        color: white;
        font-weight: 500;
        display: flex;
        justify-content: center;
...
const SignIn = () => {
  ...
  const signInAsGuestHandler = () => {
    ...
    signInWithEmailAndPassword(auth, 'guest@fakemail.com', 'guestadmin')
      .then((user) => {
      })
      .catch((error) => {
        const errorCode = error.code;
